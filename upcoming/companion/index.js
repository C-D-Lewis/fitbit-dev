...
const getEvents = (oauthData, lastAttempt) => {
  ...
  fetchCalendarEvents(oauthData.access_token, 'primary').then((eventList) => {
    console.log(`Packet size: ${JSON.stringify(eventList).length}`);
    comm.sendFile('events.json', { eventList });
  }).catch((err) => {
    console.log(err.message);
    if(lastAttempt) {
      console.log('Gave up reauthenticating');
      comm.sendFile('error.js', { error: err.message || 'Download Failed' });
      return;
    }
    ...
    auth.refreshAccess(oauthData).then((newOauthData) => {
      if(!newOauthData) {
        ...
        comm.sendFile('error.js', { error: 'Please log in' });
        return;
      }
